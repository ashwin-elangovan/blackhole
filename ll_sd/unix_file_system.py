...
# The Specification Pattern is a design pattern used in software development to encapsulate business rules that determine whether a
# certain combinat ion of data satisfies a particular criterion.
# Each concrete specification class implements the Specification interface and encapsulates a specific business rule or condition.

# Specification Interface (Filter): The Filter abstract base class serves as the specification interface.
# It defines the apply() method, which is responsible for applying the filter criterion to a file.
# This interface usually includes a method such as is_satisfied_by() or is_satisfied(), which takes an object as input and
# returns a boolean indicating whether the object meets the specified criteria.
# Here we are using apply instead of is_satisfied_by
...
class LinuxFind():
    ...
    def apply_OR_filtering(self, root):
        ...
        while queue:
            ...
            if curr_node.is_directory:
                for child in curr_node.children:
                    queue.append(child)
            else:
                for filter in self.filters:
                    if filter.apply(curr_node):
                        found_files.append(curr_node)
                        print(curr_node)
                        break
        return found_files
    ...
    def apply_AND_filtering(self, root):
        ...
        while queue:
            ...
            if curr_node.is_directory:
                ...
                is_valid = True
                for filter in self.filters:
                    if filter.apply(curr_node) == False:
                        is_valid = False
                        break
                if is_valid:
                    found_files.append(curr_node)
